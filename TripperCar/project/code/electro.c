...
void Get_Adc_Price()
{
    ...
    adcNum[0] = (float)100*(adcNum[0]-ADC_min - 15)/(ADC_max-ADC_min);          //归一化
...
void Get_Charge_Num()
{
    ChargeAdcNum = adc_mean_filter_convert(ADC1_IN12_C2, 10);
}
/*
 * @brief    差比和算法
 *
...
void Difference_Ratio_And()
{
    ...
    if(DirectionErr[0]>0.55||DirectionErr[0]<-0.55)
    {
        g_dirControl_P = 102;
        g_dirControl_D = 25;
    }
    else if(DirectionErr[0]<0.55||DirectionErr[0]>-0.55)
    {
        g_dirControl_P = 75;
        g_dirControl_D = 25;
    }
...
//    tft180_show_float(numPoint, 0, AdcDeviation, 4, 2);
    tft180_show_string(0, 0, "CD:");
    tft180_show_uint(numPoint, 0, ChargeAdcNum, 5);
