...
void bluetooth_ch9141_uart_callback()
{
    ...
    if(1 == at_mode)
    {
        ...
        at_mode_data[at_mode_num++] = ((USART_TypeDef*)UARTN[BLUETOOTH_CH9141_UART])->DATAR & 0x01FF;;
    ...
    else
    {
        ...
        // 读取无线串口的数据 并且置位接收标志
        uart_flag = 1;
        uart_data = ((USART_TypeDef*)UARTN[BLUETOOTH_CH9141_UART])->DATAR & 0x01FF;;
    }
...
{
    systick_delay_ms(600);  //发送进入AT模式的命令前需要保证模块在550ms内没有接收过任何数据
    at_mode = 1;            //进入AT模式
...
void bluetooth_ch9141_init(CH9141_MODE_enum mode, int8 *salve_mac_password)
{
    ...
    uart_rx_irq(BLUETOOTH_CH9141_UART, ENABLE);
    EnableGlobalIRQ(0);
